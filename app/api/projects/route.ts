...
export async function GET() {
  ...
  const rows = getDb().prepare(`
    SELECT p.*, ph.hp, ph.face,
           (SELECT MAX(last_msg_at) FROM sessions WHERE project_id=p.id AND is_internal=0) AS last_msg_at,
           (SELECT COUNT(*)         FROM sessions WHERE project_id=p.id AND is_internal=0) AS session_count
